...
class FiddlerDeepSeekV2:
    ...
    def __init__(self, args):
        ...
        self.expert_placeholder3 = copy.deepcopy(first_layer_mlp.experts[2]).to(self.dev)
        self.expert_placeholder4 = copy.deepcopy(first_layer_mlp.experts[3]).to(self.dev)
        self.expert_placeholder5 = copy.deepcopy(first_layer_mlp.experts[4]).to(self.dev)
        self.expert_placeholder6 = copy.deepcopy(first_layer_mlp.experts[5]).to(self.dev)
        ...
        print(f"Number of experts on GPU: {n_expert_on_gpu}/{(self.n_layer-1) * self.n_expert}")
            ...
            if i_layer == 0:
                ...
                if self.cpu_offload == 0:
                    ...
                    def process_cpu_experts():
                        ...
                        # 预热CPU专家 (首次调用会触发编译)
                        if cpu_experts:
                            dummy_input = torch.randn(1, hidden_dim, device="cpu").to(self.dtype)
                            _ = self.model.layers[i_layer].mlp.experts[cpu_experts[0]](dummy_input)
