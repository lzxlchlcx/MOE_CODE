...
class FiddlerQwen:
    ...
    def bring_expert_to_gpu(self):
        ...
        try:
            ...
            with open("test.txt", "a") as f:
                f.write(
                    f"模型: Qwen, batch_size: {self.batch_size}, 成功加载专家数量: {expert_count}\n"
                )
            ...
            if "out of memory" in str(e).lower():
                ...
                with open("test.txt", "a") as f:
                    f.write(
                        f"模型: Qwen, batch_size: {self.batch_size}, 显存溢出时专家数量: {expert_count}\n"
                    )
                raise  # 重新抛出异常
            else:
                raise  # 其他异常直接抛出
    ...
    def calc_n_expert_on_gpu(self):
        ...
        n_param = sum(
            p.numel() for p in self.model.layers[0].mlp.experts[0].parameters()
        )
    ...
    def mixtral_forward(self, input_ids, position_ids, attention_mask):
        ...
        for i_layer, layer in enumerate(self.model.layers):
            ...
            if i_layer < self.n_layer - 1:  # 如果不是最后一层
                ...
                # 使用当前层输出预测下一层专家
                with torch.no_grad():
                    next_router_logits = next_layer.mlp.gate(
                        inps
                    )  # Moon模型使用mlp.gate
                    next_routing_weights = F.softmax(next_router_logits, dim=1)
                    _, next_predicted_experts = torch.topk(
                        next_routing_weights, 8, dim=-1
                    )
