...
class FiddlerMoon:
    ...
    def bring_expert_to_gpu(self):
        ...
        try:
            ...
            with open("test.txt", "a") as f:
                f.write(
                    f"模型: moom, batch_size: {self.batch_size}, 成功加载专家数量: {expert_count}\n"
                )
            ...
            if "out of memory" in str(e).lower():
                ...
                with open("test.txt", "a") as f:
                    f.write(
                        f"模型: moom, batch_size: {self.batch_size}, 显存溢出时专家数量: {expert_count}\n"
                    )
                raise  # 重新抛出异常
            else:
                raise  # 其他异常直接抛出
    ...
    def calc_n_expert_on_gpu(self):
        ...
        print(
            f"Number of parameters in a single expert: {n_param}"
        )  # 计算一个粗粒度专家的参数量
        ...
        for i_layer, layer in enumerate(self.model.layers):
            ...
            if i_layer == 0:
                ...
                if i_layer < self.n_layer - 1:  # 如果不是最后一层
                    ...
                    # 使用当前层输出预测下一层专家
                    with torch.no_grad():
                        next_predicted_experts, next_routing_weights = (
                    ...
                    def process_gpu_experts():
                        ...
                        def process_experts_remaining():
                            ...
                            for i_expert in experts_remaining:
                                ...
                                self._async_ondemand(
                                    i_layer, i_expert, target_placeholder
                                )
